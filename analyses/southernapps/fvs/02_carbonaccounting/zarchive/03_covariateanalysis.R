...
hl.p <- mcmc_intervals(hl.mod, pars=c("b_PrevBA", 
  ...
  coord_cartesian(xlim=c(-1, 1.2), ylim=c(1, 5.15), clip = 'off') +
  ...
  annotate("segment", x = 0.01, xend = 1.19, y = 5.9, yend = 5.9, colour = "black", linewidth=0.2, arrow=arrow(length=unit(0.20,"cm"))) + 
  annotate("segment", x = -.01, xend = -0.9, y = 5.9, yend = 5.9, colour = "black", linewidth=0.2, arrow=arrow(length=unit(0.20,"cm"))) + 
  annotate("text", x = 0.6, y = 6.05, colour = "black", size=3.5, label=bquote("Increased likelihood")) + 
  annotate("text", x = -0.5, y = 6.05, colour = "black", size=3.5, label=bquote("Decreased likelihood")) +
...
hi.p <- mcmc_intervals(hi.mod, pars=c("b_PrevBA", 
                                      ...
                                      "b_PrevStdAge", "b_NumMills"), 
                       prob=0.5, prob_outer = 0.89) +
  coord_cartesian(xlim=c(-0.2, 0.2), ylim=c(1, 5.15), clip = 'off') +
  scale_x_continuous(labels = function(x) {format(100*x, digits = 2)}) +
  geom_vline(linetype = "dotted", xintercept = 0) +
  theme_bw() +
  theme(panel.border = element_rect(colour = "black", fill=NA),
        plot.margin = unit(c(3,3,1,1), "lines")) +
  annotate("segment", x = 0.001, xend = 0.19, y = 5.9, yend = 5.9, colour = "black", linewidth=0.2, arrow=arrow(length=unit(0.20,"cm"))) + 
  annotate("segment", x = -.001, xend = -0.19, y = 5.9, yend = 5.9, colour = "black", linewidth=0.2, arrow=arrow(length=unit(0.20,"cm"))) + 
  annotate("text", x = 0.1, y = 6.05, colour = "black", size=3.5, label=bquote("Increased intensity")) + 
  annotate("text", x = -0.1, y = 6.05, colour = "black", size=3.5, label=bquote("Decreased intensity")) +
  scale_y_discrete(limits=rev(c("b_PrevBA", 
                                "b_PrevLoreyHT",
                                "b_PrevSpeciesDesireability",
                                "b_PrevStdAge", "b_NumMills")), 
...
qmd.p <- mcmc_intervals(qmd.mod, pars=c("b_PrevBA", 
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA),
        plot.margin = unit(c(3,3,1,1), "lines")) +
  annotate("segment", x = 0.1, xend = .9, y = 5.9, yend = 5.9, colour = "black", linewidth=0.2, arrow=arrow(length=unit(0.20,"cm"))) + 
...
if(useoak == TRUE){
  ...
  ggplot(harv, aes(x=nummills, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$nummills>=10]) 
  mean(harv$hi)
  
  ggplot(harv, aes(x=baac.prev, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$baac.prev>=120])
  mean(harv$hi)
  ...
  ggplot(harv, aes(x=nummills, y=qmdest)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$qmdest[harv$nummills>=10]) 
  mean(harv$qmdest)
  ...
  covsub <- cov %>%
    filter(nummills >=10, baac.prev >= 100)
  ...
  harvsub <- harv %>%
    filter(harvest == 1,nummills >=10, baac.prev >= 100)
  ...
  allharv <- ggplot(harv, aes(y=hl*100, x=baac.prev)) + 
    ...
    geom_text(aes(label = paste0("mean = ",round(mean(hl*100), digits=0), "%")), y=90, x=200, col="red4")
  
  subharv <- ggplot(harvsub, aes(y=hi*100, x=baac.prev)) + 
    geom_smooth(method="lm", col="red", linetype="dashed") + geom_point(alpha=0.3) +
    xlab("Previous BA per acre (sqft)") + ylab("Harvest likelihood (%)") + coord_cartesian(ylim=c(0, 100))+
    geom_text(aes(label = paste0("mean = ",round(mean(hi*100), digits=0), "%")), y=90, x=200, col="red4")
  ...
  allremv <- ggplot(harv, aes(y=hi*100, x=nummills)) + 
    ...
    geom_text(aes(label = paste0("mean = ",round(mean(hi*100), digits=0), "%")), y=10, x=20, col="red4")
  ...
  ggplot(cov, aes(x=lorey_ht.prev, y=hl)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(cov$hl[cov$lorey_ht.prev>=70]) 
  mean(cov$hl)
  
  ggplot(cov, aes(x=desire.prev, y=hl)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(cov$hl[cov$desire.prev<=2])
  mean(cov$hl)
  ...
  ggplot(harv, aes(x=nummills, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$nummills>=7]) 
  mean(harv$hi)
  
  ggplot(harv, aes(x=lorey_ht.prev, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$baac.prev>=70])
  mean(harv$hi)
  ...
  ##### No evident trends!!!! No tweaks can be made.
