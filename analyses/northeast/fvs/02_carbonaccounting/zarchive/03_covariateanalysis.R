...
if(useoak == TRUE){
  ...
  ggplot(harv, aes(x=lorey_ht.prev, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$lorey_ht.prev>=70]) 
  mean(harv$hi)
  ...
  ggplot(harv, aes(x=baac.prev, y=qmdest)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$qmdest[harv$baac.prev>=100]) ### Very clear relationship!
  mean(harv$qmdest)
  
  
  covsub <- cov %>%
    filter(baac.prev >= 100, nummills > 0)
  
  harvsub <- harv %>%
    filter(harvest == 1, baac.prev >= 100, nummills > 0)
  
  allqmd <- ggplot(harv, aes(y=qmdest, x=baac.prev)) + 
    geom_smooth(method="lm", col="red", linetype="dashed") + geom_point(alpha=0.3) +
    ggtitle("Change in QMD after harvest") +
    xlab("Previous BA per acre (sqft)") + ylab("QMD change") + coord_cartesian(ylim=c(0,1)) +
    geom_text(aes(label = paste0("mean = ",round(mean(qmdest), digits=2))), y=0.9, x=120, col="red4")
  
  subqmd <- ggplot(harvsub, aes(y=qmdest, x=baac.prev)) + 
    geom_smooth(method="lm", col="red", linetype="dashed") + geom_point(alpha=0.3) +
    xlab("Previous BA per acre (sqft)") + ylab("QMD change") + coord_cartesian(ylim=c(0,1))+
    geom_text(aes(label = paste0("mean = ",round(mean(qmdest), digits=2))), y=0.9, x=120, col="red4")
  ...
  ggplot(cov, aes(x=lorey_ht.prev, y=hl)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(cov$hl[cov$lorey_ht.prev>=60]) 
  mean(cov$hl)
  
  ggplot(cov, aes(x=baac.prev, y=hl)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(cov$hl[cov$baac.prev>=90]) ### This is the key winner
  mean(cov$hl)
  
  ggplot(cov, aes(x=nummills, y=hl)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(cov$hl[cov$nummills>=5]) ### This is the key winner
  mean(cov$hl)
  ...
  ggplot(harv, aes(x=lorey_ht.prev, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$lorey_ht.prev>=60]) 
  mean(harv$hi)
  
  ggplot(harv, aes(x=baac.prev, y=hi)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$hi[harv$baac.prev>=90])
  mean(harv$hi)
  ...
  ggplot(harv, aes(x=lorey_ht.prev, y=qmdest)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$qmdest[harv$lorey_ht.prev>=60]) ### slightly less negative
  mean(harv$qmdest)
  
  ggplot(harv, aes(x=baac.prev, y=qmdest)) + geom_point() +
    geom_smooth(method="lm", formula=y~poly(x,2))
  mean(harv$qmdest[harv$baac.prev>=90]) 
  mean(harv$qmdest)
  ...
  covsub <- cov %>%
    filter(baac.prev >= 100, nummills > 5)
  ...
  allharv <- ggplot(harv, aes(y=hl*100, x=baac.prev)) + 
    ...
    geom_text(aes(label = paste0("mean = ",round(mean(hl*100), digits=0), "%")), y=90, x=100, col="red4")
  
  subharv <- ggplot(harvsub, aes(y=hl*100, x=baac.prev)) + 
    geom_smooth(method="lm", col="red", linetype="dashed") + geom_point(alpha=0.3) +
    xlab("Previous BA per acre (ft)") + ylab("Harvest Likelihood") + coord_cartesian(ylim=c(0,100))+
    geom_text(aes(label = paste0("mean = ",round(mean(hl*100), digits=0), "%")), y=90, x=140, col="red4")
  
  allqmd <- ggplot(harv, aes(y=qmdest, x=baac.prev)) + 
    geom_smooth(method="lm", col="red", linetype="dashed") + geom_point(alpha=0.3) +
    ggtitle("Change in QMD after harvest") +
    xlab("Previous BA per acre (sqft)") + ylab("QMD change") + coord_cartesian(ylim=c(-1,1)) +
    geom_text(aes(label = paste0("mean = ",round(mean(qmdest), digits=2))), y=0.9, x=120, col="red4")
